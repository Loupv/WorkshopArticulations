...
void draw() {
  ...
  for (int listInd = 0; listInd < dataList.size(); listInd++)
  {
    // To change the colors you should define here a variable that depends on the listInd. Basically, listInd 0 means data from one harness, and listInd 1 is from another. If you then give your variable as the first value to the stroke methods below, you will change the hues of the graphs.
    ArrayList<SummarySample> data = dataList.get(listInd);
    if (data.size() > 1)
    {
      ...
      for (int i = 1; i < data.size(); i++)
      {
        ...
        {
          //stroke(10, 100, 100);
          //line(width - ((xStart - sum0.time) / plottedMs * width),
          //      (sum0.batteryLevel * -10.0) + (height * 0.99), 
          //      width - ((xStart - sum1.time)  / plottedMs * width), 
